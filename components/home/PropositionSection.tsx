  ...
  const valueProps = [
    {
      ...
      description:
        'Ich werde dein Tech-Co-Founder. Nicht nur Code, sondern strategische Partnerschaft mit Equity oder Revenue Share.',
      cta: 'Partner werden',
      href: '/partnerships',
      features: [
        'Equity Beteiligung',
    ...
    },
    {
      ...
      description:
        'Verkaufe dein Web-Projekt an mich. Ich optimiere, skaliere und führe es weiter - fairer Deal, reibungsloser Übergang.',
      cta: 'Projekt verkaufen',
      href: '/acquisition',
      features: [
        'Sofortige Auszahlung',
        'Fairer Bewertungsprozess',
    ...
    },
    {
      ...
      description:
        'Dein Projekt läuft, aber nicht optimal? Ich mache es schneller, stabiler und profitabler.',
    ...
    },
    {
      ...
      description:
        'Gemeinsam bauen wir nicht nur eine Webseite, sondern ein wertvolles digitales Asset für Exit oder Passive Income.',
      cta: 'Mehr erfahren',
      href: '/portfolio',
      features: [
        'Wertsteigerungs-Plan',
        'Exit-Strategie',
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        />
        <div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-amber-500/5 rounded-full blur-2xl animate-pulse"
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">
            Technische Expertise meets Business-Mindset. Wir erschaffen nicht
            nur Software, sondern{' '}
            <span className="text-amber-400 font-semibold">
              wertvolle digitale Assets
            </span>{' '}
            mit klarer Exit-Strategie.
          </p>
        </div>
          ...
          {valueProps.map((prop, index) => {
            ...
            return (
              ...
              >
                {/* Hover Glow Effect */}
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${prop.gradient} rounded-xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${colors.glow}`}
                />

                {/* Main Card */}
                <div
                  className={`relative p-8 bg-slate-800/50 border border-slate-700 rounded-xl backdrop-blur-sm transition-all duration-500 group-hover:${colors.border} group-hover:bg-slate-800/80 group-hover:-translate-y-3 h-full flex flex-col ${hoveredCard === index ? 'scale-105' : ''}`}
                >
                  {/* Animated Icon */}
                  <div
                    className={`w-16 h-16 bg-gradient-to-br ${prop.gradient} rounded-2xl flex items-center justify-center mb-6 ${colors.text} group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 border ${colors.border}`}
                  >
                    {prop.icon}
                  </div>
                ...
                {/* Floating Number */}
                <div
                  className={`absolute -top-3 -left-3 w-8 h-8 ${colors.bg} rounded-full flex items-center justify-center border-2 border-slate-900 group-hover:scale-110 group-hover:rotate-12 transition-transform duration-500 ${colors.text} font-bold text-sm`}
                >
                  {index + 1}
                </div>
              </div>
