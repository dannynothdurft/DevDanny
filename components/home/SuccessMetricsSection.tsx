...
import { TrendingUp, Users, Gem, Clock, Target, Award, Sparkles, BarChart3 } from "lucide-react"
  ...
  return (
    <section ref={sectionRef} className="py-20 bg-gradient-to-b from-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-10 right-10 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-purple-500/5 rounded-full blur-2xl animate-pulse" style={{ animationDelay: '4s' }}></div>
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center mb-4 px-4 py-2 bg-green-500/10 border border-green-500/30 rounded-full backdrop-blur-sm">
            ...
            <span className="text-sm text-green-400 font-medium">Bewährte Track Record</span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4 text-white">
            Zahlen, die <span className="bg-gradient-to-r from-green-400 via-cyan-400 to-blue-400 bg-clip-text text-transparent">überzeugen</span>
          </h2>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">
            Transparente Metriken aus realen Projekten. Jede Zahl erzählt eine Erfolgsgeschichte.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 max-w-7xl mx-auto">
          {successMetrics.map((item, index) => (
            ...
            >
              ...
              <div className={`relative p-6 ${item.bgColor} border ${item.borderColor} rounded-2xl backdrop-blur-sm transition-all duration-500 group-hover:scale-105 group-hover:-translate-y-2 h-full flex flex-col items-center text-center`}>
                
                {/* Animated Icon */}
                <div className={`w-14 h-14 bg-gradient-to-br ${item.color} rounded-2xl flex items-center justify-center mb-4 text-white group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-lg`}>
                  {item.icon}
                </div>

                {/* Animated Number */}
                <div className={`text-3xl font-black bg-gradient-to-r ${item.color} bg-clip-text text-transparent mb-2 transition-all duration-300 min-h-[2.5rem] flex items-center justify-center`}>
                  {isVisible ? countedMetrics[index] || 0 : 0}{item.suffix}
                </div>
              ...
              {index < successMetrics.length - 1 && (
                <>
                  <div className="hidden xl:block absolute top-1/2 -right-3 w-6 h-0.5 bg-gradient-to-r from-gray-600 to-transparent">
                    <div className={`h-full bg-gradient-to-r ${item.color} transition-all duration-1000 delay-500 ${
                      isVisible && countedMetrics[index] >= item.metric ? 'w-full' : 'w-0'
                    }`} />
                  </div>
                </>
              )}
        ...
        <div className="text-center mt-12">
          ...
          >
            <span className="font-semibold">Erfolgsgeschichten im Detail ansehen</span>
